...
{
	if (is_builtin_op(sym))
		return;
	for (int i = 0; i < custom_op_count; i++)
		if (strcmp(custom_ops[i], sym) == 0)
			return;
...
static void prescan_custom_ops(const char *src, size_t len)
{
	for (size_t i = 0; i + 8 < len; i++) {
		if (strncmp(src + i, "operator", 8) != 0)
			continue;
		if (i > 0 &&
		    (isalnum((unsigned char)src[i - 1]) || src[i - 1] == '_'))
			continue;
		size_t p = i + 8;
		if (p < len &&
		    (isalnum((unsigned char)src[p]) || src[p] == '_'))
			continue;
		while (p < len && isspace((unsigned char)src[p]))
			p++;
		size_t start = p;
		while (p < len && is_op_char(src[p]))
			p++;
		if (p > start) {
...
static const char *match_custom_op(const char *src, size_t pos, size_t len)
{
	...
	for (int i = 0; i < custom_op_count; i++) {
		...
		if (n > best_len && pos + n <= len &&
		    strncmp(src + pos, custom_ops[i], n) == 0) {
			best = custom_ops[i];
			best_len = n;
		}
	}
...
static void push_tok(TokBuf *b, Token t)
{
	if (b->count == b->cap) {
		...
		b->items = realloc(b->items, b->cap * sizeof(Token));
	}
	b->items[b->count++] = t;
}
			      ...
			      int *line)
{
	...
	while (*pos < len && src[*pos] != '"') {
		...
		if (c == '$' && *pos + 1 < len && src[*pos + 1] == '{') {
			...
			size_t chunk_len =
				p - start + 1; /* include closing brace */
			if (n + chunk_len >= cap) {
				cap = (n + chunk_len) * 2;
				out = realloc(out, cap);
			}
...
Token *lex_all(const char *src, int *out_count)
{
	...
	while (pos < len) {
		...
		if (isdigit((unsigned char)c)) {
			...
			Token t =
				make(is_float ? T_FLOAT : T_INT, numstr, line);
			if (is_float)
				t.dval = strtod(numstr, NULL);
			else
				t.ival = atoll(numstr);
			free(numstr);
			push_tok(&buf, t);
		...
		if (isalpha((unsigned char)c) || c == '_') {
			size_t start = pos;
			while (pos < len && (isalnum((unsigned char)src[pos]) ||
					     src[pos] == '_'))
				pos++;
			...
			for (int i = 0; keywords[i].word; i++) {
				if (strcmp(keywords[i].word, ident) == 0) {
					kw = keywords[i].type;
					found = true;
					break;
				}
		...
		switch (c) {
		case '{':
			push_tok(&buf, make(T_LBRACE, "{", line));
			pos++;
			continue;
		case '}':
			push_tok(&buf, make(T_RBRACE, "}", line));
			pos++;
			continue;
		case '(':
			push_tok(&buf, make(T_LPAREN, "(", line));
			pos++;
			continue;
		case ')':
			push_tok(&buf, make(T_RPAREN, ")", line));
			pos++;
			continue;
		case '[':
			push_tok(&buf, make(T_LBRACKET, "[", line));
			pos++;
			continue;
		case ']':
			push_tok(&buf, make(T_RBRACKET, "]", line));
			pos++;
			continue;
		case ',':
			push_tok(&buf, make(T_COMMA, ",", line));
			pos++;
			continue;
		case ':':
			push_tok(&buf, make(T_COLON, ":", line));
			pos++;
			continue;
		case ';':
			push_tok(&buf, make(T_SEMI, ";", line));
			pos++;
			continue;
		case '+':
			push_tok(&buf, make(T_PLUS, "+", line));
			pos++;
			continue;
		case '-': {
			if (pos + 1 < len && src[pos + 1] == '>') {
				push_tok(&buf, make(T_ARROW, "->", line));
				pos += 2;
				continue;
			}
			push_tok(&buf, make(T_MINUS, "-", line));
			pos++;
			continue;
		}
		case '*':
			push_tok(&buf, make(T_STAR, "*", line));
			pos++;
			continue;
		case '%':
			push_tok(&buf, make(T_PERCENT, "%", line));
			pos++;
			continue;
		case '.':
			push_tok(&buf, make(T_DOT, ".", line));
			pos++;
			continue;
		case '/':
			push_tok(&buf, make(T_SLASH, "/", line));
			pos++;
			continue;
		case '=':
			if (pos + 1 < len && src[pos + 1] == '=') {
				push_tok(&buf, make(T_EQ, "==", line));
				pos += 2;
				continue;
			}
			push_tok(&buf, make(T_ASSIGN, "=", line));
			pos++;
			continue;
		case '!':
			if (pos + 1 < len && src[pos + 1] == '=') {
				push_tok(&buf, make(T_NEQ, "!=", line));
				pos += 2;
				continue;
			}
			push_tok(&buf, make(T_NOT, "!", line));
			pos++;
			continue;
		case '<':
			if (pos + 1 < len && src[pos + 1] == '=') {
				push_tok(&buf, make(T_LTE, "<=", line));
				pos += 2;
				continue;
			}
			if (pos + 1 < len && src[pos + 1] == '<') {
				push_tok(&buf, make(T_SHL, "<<", line));
				pos += 2;
				continue;
			}
			push_tok(&buf, make(T_LT, "<", line));
			pos++;
			continue;
		case '>':
			if (pos + 1 < len && src[pos + 1] == '=') {
				push_tok(&buf, make(T_GTE, ">=", line));
				pos += 2;
				continue;
			}
			if (pos + 1 < len && src[pos + 1] == '>') {
				push_tok(&buf, make(T_SHR, ">>", line));
				pos += 2;
				continue;
			}
			push_tok(&buf, make(T_GT, ">", line));
			pos++;
			continue;
		case '&':
			if (pos + 1 < len && src[pos + 1] == '&') {
				push_tok(&buf, make(T_ANDAND, "&&", line));
				pos += 2;
				continue;
			}
			push_tok(&buf, make(T_AMP, "&", line));
			pos++;
			continue;
		case '|':
			if (pos + 1 < len && src[pos + 1] == '|') {
				push_tok(&buf, make(T_OROR, "||", line));
				pos += 2;
				continue;
			}
			push_tok(&buf, make(T_PIPE, "|", line));
			pos++;
			continue;
		case '^':
			push_tok(&buf, make(T_CARET, "^", line));
			pos++;
			continue;
		case '~':
			push_tok(&buf, make(T_TILDE, "~", line));
			pos++;
			continue;
		case '?':
			if (pos + 1 < len && src[pos + 1] == '?') {
				push_tok(&buf, make(T_QQ, "??", line));
				pos += 2;
				continue;
			}
			if (pos + 1 < len && src[pos + 1] == '.') {
				push_tok(&buf, make(T_QDOT, "?.", line));
				pos += 2;
				continue;
			}
			push_tok(&buf, make(T_QUESTION, "?", line));
			pos++;
			continue;
		default:
			fprintf(stderr,
				"oboe: unexpected character '%c' at line %d\n",
				c, line);
			exit(1);
		}
	}
