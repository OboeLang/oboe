...
static void walk_object(const char *obj, KeyVisitor visit, void *ctx) {
    ...
    for (const char *p = start; *p; p++) {
        ...
        if (*p == '"') { in_string = true; key_start = p + 1; continue; }
        else if (*p == '{' || *p == '[') depth++;
        else if (*p == '}' || *p == ']') depth--;
...
static void find_visitor(const char *key, size_t key_len, const char *value, void *ctx) {
    FindCtx *f = ctx;
    if (f->found) return;
    if (strlen(f->want) == key_len && strncmp(key, f->want, key_len) == 0) f->found = value;
}
...
static void keys_visitor(const char *key, size_t key_len, const char *value, void *ctx) {
    (void)value;
    KeysCtx *k = ctx;
    if (k->count == k->cap) {
